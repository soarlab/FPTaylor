...
let print_double_f env f_name fmt expr =
  clear_exprs env;
...
let generate_error_bounds fmt task =
  ...
  pp_print_newline fmt ();
  print_init_f_and_mps env fmt ~double:true ~single:true ~mpfi:true [spec_expr; expr];
  fprintf fmt "@.#else@.";
  fprintf fmt "@.#include \"search_mpfr.h\"@.";
  pp_print_newline fmt ();
  print_init_f_and_mps env fmt ~double:true ~single:true ~mpfi:false [spec_expr; expr];
  fprintf fmt "@.#endif@.";
  pp_print_newline fmt ();
  ...
  pp_print_newline fmt ();
  print_double_f env "f_64" fmt expr;
  pp_print_newline fmt ();
  print_double_f env "f_64_high" fmt spec_expr;
  pp_print_newline fmt ();
